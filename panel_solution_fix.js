// === PANEL DE PROPIEDADES: SOLUCIÃ“N DEFINITIVA ===

// 1. DelegaciÃ³n robusta para selecciÃ³n de WIDGET
$('#sections-list').off('click', '.elementor-widget').on('click', '.elementor-widget', function(e) {
    ...
    e.stopPropagation(); // Evita que el clic llegue a elementos padres (columna, secciÃ³n)
    ...
    console.log(`PANEL: Buscando widget en Ã­ndices [${sidx}][${cidx}][${widx}]`); 
    ...
    try {
        // --- GeneraciÃ³n del HTML del formulario de propiedades ---
        ...
        if(selectedWidgetRef.type === 'select' || selectedWidgetRef.type === 'radio') {
            html += `<div class='mb-2'><label class='form-label fw-bold'>Opciones (una por lÃ­nea)</label><textarea class='form-control' name='options' rows='3'>${(selectedWidgetRef.options||[]).join('\n')}</textarea></div>`;
        }
        ...
        $('#properties-content').html('<div class="alert alert-danger">Error crÃ­tico al procesar el clic del widget. Revisa la consola.</div>');
    }
});

// 2. DelegaciÃ³n para GUARDAR propiedades desde el panel
$(document).off('submit', '#widget-properties-form').on('submit', '#widget-properties-form', function(e) {
    ...
    try {
        ...
        // Guardar los Ã­ndices antes de renderizar
        ...
        // Intentar reseleccionar el widget despuÃ©s de renderizar
        setTimeout(() => {
            try {
        ...
        alert('OcurriÃ³ un error al guardar: ' + error.message);
    }
});

// 3. DelegaciÃ³n para selecciÃ³n de COLUMNA (muestra panel bÃ¡sico)
$('#sections-list').off('click', '.elementor-column').on('click', '.elementor-column', function(e) {
    ...
    $(this).addClass('selected');
    $('#properties-panel').show().css('right', '0px').addClass('active');
    $('#properties-content').html('<p class="text-muted">Propiedades de la Columna seleccionada. (EdiciÃ³n futura)</p>');
});

// 4. DelegaciÃ³n para selecciÃ³n de SECCIÃ“N (muestra panel bÃ¡sico)
$('#sections-list').off('click', '.elementor-section').on('click', '.elementor-section', function(e) {
    e.stopPropagation();
    console.log('PANEL: Clic detectado en SecciÃ³n:', this);
    $('.elementor-widget, .elementor-column, .elementor-section').removeClass('selected');
    $(this).addClass('selected');
    $('#properties-panel').show().css('right', '0px').addClass('active');
    $('#properties-content').html('<p class="text-muted">Propiedades de la SecciÃ³n seleccionada. (EdiciÃ³n futura)</p>');
});
...
// === FIN SOLUCIÃ“N DEFINITIVA PANEL ===
