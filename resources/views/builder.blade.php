...
<body>
{{-- Forzar UTF-8 para tildes y caracteres especiales --}}
<meta charset="utf-8">

{{-- MENÚ RESPONSIVO BOOTSTRAP --}}
<script>
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="mainNavbar">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="#">Builder</a>
        </li>
        <li class="nav-item">
      ...
      <button class="btn btn-light btn-sm ms-2" onclick="toggleMenuType()">Cambiar menú: Superior/Lateral</button>
      <span class="navbar-text d-none d-lg-inline ms-3">Generador de formularios</span>
    </div>
  </div>
</nav>

<div id="sidebar-menu" style="display:none; position:fixed; top:0; left:0; height:100vh; width:220px; background:#0d6efd; color:#fff; z-index:1050;">
  <div class="d-flex flex-column h-100">
    <div class="p-3 border-bottom"><span class="fw-bold">FormBox</span></div>
    <ul class="nav flex-column px-2 pt-2">
      <li class="nav-item"><a class="nav-link text-white active" href="#">Builder</a></li>
      <li class="nav-item"><a class="nav-link text-white" href="#">Formularios</a></li>
    ...
    <div class="mt-auto p-3">
      <button class="btn btn-light btn-sm w-100" onclick="toggleMenuType()">Cambiar menú: Superior/Lateral</button>
      <div class="small mt-2">Generador de formularios</div>
    </div>
  </div>
</div>
...
@media (max-width: 991px) {
  #sidebar-menu { width:100vw; position:fixed; z-index:2000; }
}
body.menu-side #sidebar-menu { display:block; }
</style>
...
<div class="container mt-5">
    ...
    <div class="row mt-4">
        <div class="col-12 text-end">
            <input type="text" id="form-name" class="form-control d-inline-block w-auto me-2" placeholder="Nombre del formulario" style="max-width:220px;">
            <input type="text" id="form-table-name" class="form-control d-inline-block w-auto me-2" placeholder="Nombre de tabla" style="max-width:180px;">
            <button class="btn btn-success" id="btn-save-form"><i class="bi bi-save"></i> Guardar formulario</button>
            <button class="btn btn-outline-primary ms-2" id="btn-login-api"><i class="bi bi-key"></i> Login API</button>
        </div>
    </div>
    <div class="row mt-4">
        <div class="col-12">
...
<!-- Modal de login API -->
<div class="modal fade" id="apiLoginModal" tabindex="-1" aria-labelledby="apiLoginModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="apiLoginModalLabel">Login API</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
          <label for="api-email" class="form-label">Email</label>
          <input type="email" class="form-control" id="api-email" placeholder="usuario@correo.com">
        </div>
        <div class="mb-3">
          <label for="api-password" class="form-label">Contraseña</label>
          <input type="password" class="form-control" id="api-password" placeholder="Contraseña">
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
        <button type="button" class="btn btn-primary" id="btn-api-login-submit">Iniciar sesión</button>
      </div>
    </div>
  </div>
</div>
...
function renderPropertiesPanel() {
    ...
    if (field.type === 'url') {
        ...
        html += `<div class='mb-2'><label class='form-label'>Opciones (separadas por coma)</label><input type='text' class='form-control' name='options' value='${(field.options||[]).join(", ")}' placeholder='Ej: Opción 1, Opción 2'></div>`;
    } else if (field.type === 'tel') {
...
$(function() {
    if ($('#properties-panel').length === 0) {
        $('#properties').html('<div id="properties-panel"></div>');
    }
    renderPropertiesPanel();
    ...
    renderPreview();
    initPanelDraggables();
    // Eventos para botones de agregar/quitar filas/columnas
    ...
    `).appendTo('head');
    // Mostrar modal login API
    $(document).on('click', '#btn-login-api', function() {
        $('#apiLoginModal').modal('show');
    });
    // Login API
    $(document).on('click', '#btn-api-login-submit', function() {
        const email = $('#api-email').val().trim();
        const password = $('#api-password').val();
        if (!email || !password) {
            Swal.fire('Campos requeridos', 'Debes ingresar email y contraseña.', 'warning');
            return;
        }
        $.ajax({
            url: '/formbox/public/api/login',
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({ email, password }),
            success: function(response) {
                localStorage.setItem('api_token', response.token);
                Swal.fire('¡Listo!', 'Token guardado en localStorage.', 'success');
                $('#apiLoginModal').modal('hide');
            },
            error: function(xhr) {
                Swal.fire('Error', 'Credenciales incorrectas o error de red.', 'error');
            }
        });
    });
    // Guardar formulario
    $(document).on('click', '#btn-save-form', function() {
        ...
        const tableName = $('#form-table-name').val().trim();
        if (!formName || !tableName) {
            Swal.fire('Campos requeridos', 'Debes ingresar el nombre del formulario y de la tabla.', 'warning');
            return;
        }
        // Generar los campos con name válido
        const campos = fields.map(f => ({
            name: f.name_html || sanitizeName(f.label || ''),
            type: f.type,
            required: !!f.required,
            label: f.label || ''
        }));
        // Validar que todos los campos tengan name único
        const names = campos.map(c => c.name);
        const hasDuplicates = names.length !== new Set(names).size;
        if (hasDuplicates) {
            Swal.fire('Error', 'Hay campos con nombres repetidos. Modifica los nombres para que sean únicos.', 'error');
            return;
        }
        // Enviar AJAX
        $.ajax({
            url: '/formbox/public/api/forms',
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
                name: formName,
            ...
            error: function(xhr) {
                let msg = 'Error al guardar el formulario.';
